 ...
 */
@ChannelMessageHandler('get_var', true, calGetVarRequestSchema, 'vars_access')
@injectable()
export class GetVarHandler implements ClientMessageHandler<CALGetVarRequest> {
  ...
  async handle(context: ClientMessageHandlerContext, message: CALGetVarRequest): Promise<void> {
    logger.info({ sessionId: context.session?.id, conversationId: message.conversationId, stageId: message.stageId, variableName: message.variableName, correlationId: message.correlationId }, 'Get variable request received');
