...
async function main() {
  ...
  console.log('Frame | Prob  | Energy (raw PCM)');
  console.log('------|-------|-----------------');
  ...
  // Find frames with prob > 0.1
  const speechFrames = probs.map((p, i) => ({ frame: i, prob: p })).filter(x => x.prob > 0.1);
  console.log('\nFrames with prob > 0.1:', speechFrames.length);
  if (speechFrames.length > 0) {
    console.log('First:', speechFrames[0]);
    console.log('Last:', speechFrames[speechFrames.length - 1]);
  }
