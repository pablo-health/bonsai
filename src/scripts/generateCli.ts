...
function generateCommandsFile(resources: Map<string, ResourceDef>): string {
  ...
  code += "import { RESOURCES, getResourceNames } from './resources.js';\n";
  ...
  code += '              { method: op.method, pathTemplate: op.pathTemplate, scope: res.scope, action: op.action, pathParamNames: op.pathParams.map(p => p.name), queryParamNames: op.queryParamNames, bodySchemaRef: op.bodySchemaRef }\n';
  code += '            );\n';
