...
async function main() {
  const raw = await readFile('/tmp/libri_test.pcm');
  ...
  console.log(`libri_test.pcm: samples=${pcm16.length}, range=[${minVal}, ${maxVal}], duration=${(pcm16.length / 16000).toFixed(2)}s`);
