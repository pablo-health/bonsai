...
export class ConversationRunner {
   ...
   * resumed (the model was right) or the timer expired (it was wrong). That is a free, perfectly
   * labelled dataset generated by ordinary use, and it was being thrown away - so nobody could
   * say whether any threshold here was correct, only that it felt better.
   */
  private pendingTurnPrediction: {
  ...
  private setBargeInSilenceTimer(): void {
    ...
    this.bargeInSilenceTimer = setTimeout(async () => {
      ...
      try {
        // stop() promotes any pending partial to a final chunk, which fires onRecognitionStopped
        // and submits the turn. Calling triggerBargeInSilenceResponse() unconditionally after it
        // therefore submitted the turn TWICE whenever the caller had said anything at all: once
        // with their words, then again with the unintelligible placeholder. The placeholder is
        // for the case it was written for - genuine silence, nothing transcribed.
        const hadSpeech = (this.stageData.asrProvider?.getAllTextChunks() ?? [])
          .some((chunk) => chunk.text.trim().length > 0);
        await this.stageData.asrProvider?.stop();
        if (!hadSpeech) {
          await this.triggerBargeInSilenceResponse();
        }
  ...
  private async handleSmartTurnDetection(threshold: number): Promise<boolean> {
    ...
    try {
      ...
      this.pendingTurnPrediction = {
        endpointProbability: result.endpointProbability,
        threshold,
        waitMs,
        partial: this.latestPartialText,
        at: Date.now(),
      };
      logger.info(
      ...
      this.smartTurnContinueTimer = setTimeout(async () => {
        ...
        if (this.conversation.status === 'receiving_user_voice' && this.stageData.asrProvider) {
          try {
            ...
            this.gradeTurnPrediction('finished', conversationId);
          } catch (error) {
            logger.warn(
  ...
  private smartTurnContinueWaitMs(endpointProbability: number, threshold: number): number {
    ...
    // Multiply, do not Math.max. The previous form was
    //   Math.max(uncertainty, textFactor * uncertainty, textFactor === 1 ? 1 : 0)
    // and since textFactor <= 1 the second term can never exceed the first, so it reduced to
    //   textFactor === 1 ? 1 : uncertainty
    // - the punctuation branch was arithmetically unreachable and the text signal could only ever
    // lengthen the wait, never shorten it, which is the opposite of half of what it is for.
    const weight = Math.max(0, Math.min(1, uncertainty * textFactor * 2));
  ...
  private clearSmartTurnContinueTimer(): void {
    if (this.smartTurnContinueTimer) {
      ...
      this.smartTurnContinueTimer = null;
      // Cleared means the caller started talking again before the timer fired, which is the only
      // reason it gets cleared - so the continuation verdict was correct and the wait was earned.
      this.gradeTurnPrediction('resumed', this.stageData.conversation.id);
    }
  }

  /**
   * Writes down whether a continuation verdict turned out to be right.
   *
   * One line per decision, greppable as turn_prediction, carrying everything needed to tune this
   * offline: what the audio model said, what the transcript looked like, how long we chose to
   * wait, and what the caller actually did. Two questions it answers that nothing currently can:
   * whether endpointProbability is bimodal - if almost every continuation lands near zero then
   * the whole adaptive range is unused and the tuning is theatre - and which trailing words earn
   * their place in the dangling list rather than merely sounding plausible.
   *
   * No PHI: the transcript tail is capped at 40 characters and this is a machine's guess about
   * turn-taking, not clinical content. Trim it further if that ever stops being true.
   */
  private gradeTurnPrediction(outcome: 'resumed' | 'finished', conversationId: string): void {
    const p = this.pendingTurnPrediction;
    if (!p) return;
    this.pendingTurnPrediction = null;
    logger.info({
      conversationId,
      kind: 'turn_prediction',
      outcome,
      correct: outcome === 'resumed',
      endpointProbability: Number(p.endpointProbability.toFixed(4)),
      threshold: p.threshold,
      waitMs: p.waitMs,
      elapsedMs: Date.now() - p.at,
      tail: p.partial.trim().slice(-40),
      dangling: DANGLING_TAIL.test(p.partial.trim()),
      punctuated: /[.!?]$/.test(p.partial.trim()),
    }, 'Turn prediction graded');
  }
